...
class ArgsTest : public Test {
  public:
    void pass(
            const args::Parser& parser, const char* argv0 = NULL, const char* argv1 = NULL,
            const char* argv2 = NULL, const char* argv3 = NULL, const char* argv4 = NULL,
            const char* argv5 = NULL, const char* argv6 = NULL, const char* argv7 = NULL,
            const char* argv8 = NULL) {
        ...
        EXPECT_THAT(parser.parse_args(argc, argv, error), Eq(true))
                << error << " (" << argv0 << " " << argv1 << " " << argv2 << " " << argv3 << " "
                << argv4 << " " << argv5 << " " << argv6 << " " << argv7 << " " << argv8 << ")";
    }

    void fail(
            const char* message, const args::Parser& parser, const char* argv0 = NULL,
            const char* argv1 = NULL, const char* argv2 = NULL, const char* argv3 = NULL,
            const char* argv4 = NULL, const char* argv5 = NULL, const char* argv6 = NULL,
...
struct ShortOptions {
    ...
    void add_to(args::Parser& parser) {
        parser.add_argument("-n", store_const(commit, false)).help("dry run");
        ...
        parser.add_argument("-k", store_const(units, 1000)).help("print units in kilobytes");
        parser.add_argument("-m", store_const(units, 1000000)).help("print units in megabytes");
        parser.add_argument("-g", store_const(units, 1000000000)).help("print units in gigabytes");
        ...
        parser.add_argument("-v", increment(verbosity)).help("increase verbosity level");

        parser.add_argument("-x", store(extension)).metavar("EXT").help("use extension");

        parser.add_argument("-i", store(input)).metavar("FILE").help("input file");

        parser.add_argument("-o", store(output)).metavar("FILE").help("output file");

        parser.add_argument("-t", store(type)).metavar("TYPE").help("type of output");

        parser.add_argument("-q", store(quality)).metavar("QUALITY");
    }
...
TEST_F(ArgsTest, ShortOptionsHelp) {
    ...
    const char* usage = "short [-!?gkmnvæ秋] [-i FILE] [-o FILE] [-q QUALITY] [-t TYPE] [-x EXT]";
...
struct Greeter {
    ...
    void add_to(args::Parser& parser) {
        ...
        parser.add_argument("--hello", store_const(_greeting, "Hello")).help("greet in English");
        parser.add_argument(
        ...
        parser.add_argument("--name", store(_name)).help("name of person to greet");
...
struct ArgumentsOnly {
    ...
    void add_to(args::Parser& parser) {
        parser.add_argument("one", store(one)).help("first argument").required();
        parser.add_argument("two", store(two));
        parser.add_argument("three", append(three)).help("third argument").max_args(3);
    }
...
TEST_F(ArgsTest, ArgumentsFail) {
    ...
    fail("argument three: integer overflow: \"2147483648\"", parser, "1", "2", "3", "4",
         "2147483648");
    fail("extra arguments found: \"6\"", parser, "1", "2", "3", "4", "5", "6");
    fail("extra arguments found: \"6\" \"7\" \"8\"", parser, "1", "2", "3", "4", "5", "6", "7",
         "8");
}
...
class CutTool {
  public:
    ...
    void add_to(args::Parser& parser) {
        parser.add_argument("string", store(_string)).required().help("string to split");
        parser.add_argument("-l", "--limit", store(_limit)).help("split at most this many times");
...
TEST_F(ArgsTest, CutHelp) {
    ...
    const char* usage = "cut [-d DELIM] [-l LIMIT] [--delimiter=DELIM] [--limit=LIMIT] string";
    EXPECT_THAT(parser.usage(), PrintsAs(utf8::decode(usage)));
...
struct Calculator {
    ...
    void add_to(args::Parser& parser) {
        parser.add_argument("x", store(_x)).help("first argument").required();

        parser.add_subparser("abs", "take absolute value", store_const(_op, 'a'));

        args::Parser& plus = parser.add_subparser("plus", "add arguments", store_const(_op, '+'));
        plus.add_argument("y", store(_y)).help("second argument").required();

        args::Parser& div = parser.add_subparser("div", "divide arguments", store_const(_op, '/'));
        div.add_argument("--int", store_const(_int_division, true));
        div.add_argument("y", store(_y)).help("second argument").required();
    }

    double value() const {
        double result;
        switch (_op) {
            case 'a': result = abs(_x); break;
...
TEST_F(ArgsTest, CalculatorHelp) {
    ...
    const char* usage = "calc x [command]";
    EXPECT_THAT(parser.usage(), PrintsAs(utf8::decode(usage)));
