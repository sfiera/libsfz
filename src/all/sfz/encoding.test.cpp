...
TEST_F(AsciiEncodingTest, DecodeValid) {
    ...
    SFZ_FOREACH(int i, range(string.size()), {
...
TEST_F(AsciiEncodingTest, EncodeValid) {
    ...
    SFZ_FOREACH(size_t i, range(bytes.size()), {
...
TEST_F(AsciiEncodingTest, DecodeInvalid) {
    Bytes bytes;
    SFZ_FOREACH(int i, range(0x80, 0x100), {
        bytes.append(1, i);
    });
...
TEST_F(AsciiEncodingTest, EncodeInvalid) {
    String string;
    SFZ_FOREACH(int i, range(0x8, 0x100), {
        string.append(1, i * 0x10);
    });

    Bytes bytes(ascii::encode(string));
    ASSERT_THAT(bytes.size(), Eq(string.size()));
    SFZ_FOREACH(int i, range(bytes.size()), {
...
TEST_F(Latin1EncodingTest, EncodeInvalid) {
    String string;
    SFZ_FOREACH(int i, range(0x1, 0x100), {
...
TEST_F(Utf8EncodingTest, EncodeCJK) {
    ...
    String string;
    SFZ_FOREACH(int i, range(0x3041, 0x3094), {
        string.append(1, i);
    });
...
TEST_F(Utf8EncodingTest, EncodeHigherPlane) {
    ...
    String string;
    SFZ_FOREACH(int i, range(0x1f000, 0x1f02a), {
        string.append(1, i);
    });
...
TEST_F(Utf8EncodingTest, DecodeInvalid) {
    ...
    const char kInvalid[][5] = {
        ...
        // anything, start a multi-byte code point that decoded to a value greater than 0x10ffff.
        "\365", "\366", "\367", "\370", "\371", "\372", "\373", "\374", "\375", "\376", "\377",

        // Overlong encoding.  These strings all define interpretable multi-byte code points, but
        ...
        // multi-byte character longer than the string being decoded.  Decoding should end early.
        "\300",
        "\337",
        "\340", "\340\200",
        "\357", "\357\277",
        "\360", "\360\200", "\360\200\200",
        "\364", "\364\277", "\364\277\277",

        // Premature end of code point.  Each of the first characters in this string declares a
        // multi-byte character equal to the length of the string being decoded.  Decoding should
        // continue with the valid bytes.
        "\300A",
        "\337A",
        "\340AB", "\340\200A",
        "\357AB", "\357\277A",
        "\360ABC", "\360\200AB", "\360\200\200A",
        "\364ABC", "\364\277AB", "\364\277\277A",

        // Continuation bytes not preceded by a header byte.
        "\200", "\201", "\220", "\270", "\277",

        // Surrogate code points.  Strings may not contain code points in the range 0xd800..0xdfff.
    ...
    SFZ_FOREACH(BytesSlice bytes, kInvalid, {
        String string(utf8::decode(bytes));
        ASSERT_THAT(string.size(), Eq(bytes.size()));
        SFZ_FOREACH(size_t i, range(string.size()), {
            if (bytes.at(i) < 0x80) {
